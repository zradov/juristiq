...
from llm_utils import get_contract_review_prompt_builder, get_openai_client
...
def get_user_balance() ->  Tuple[float, str]:
    ...
    """
    conn = http.client.HTTPSConnection(config.DEEP_SEEK_API_DOMAIN)
    headers = {
        "Authorization": f"Bearer {config.DEEP_SEEK_API_KEY}"
    }
...
def log_user_balance() -> None:

    balance, currency = get_user_balance()
    logger.info(f"Current user balance: {balance:.5f} {currency}")


def log_chat_metrics(chat_completion: ChatCompletion) -> None:
    """
    Logs the response metrics of the chat completion request.

    Args:
        chat_completion: result of a chat completion request.

    Returns:
        None
    """
    logger.info((f"Chat metrics: completion_tokens={chat_completion.usage.completion_tokens}, prompt_tokens="
                 f"{chat_completion.usage.prompt_tokens}, total_tokens={chat_completion.usage.total_tokens}, "
                 f"cached_tokens={chat_completion.usage.prompt_tokens_details.cached_tokens}, "
                 f"prompt_cache_hit_tokens={chat_completion.usage.prompt_cache_hit_tokens}, "
                 f"prompt_cache_miss_tokens={chat_completion.usage.prompt_cache_miss_tokens}"
...
def review_annots(transformed_cuad_annots_path: str,
    ...
    Args:
        ...
        genai_provider: a name of the GenAI provider.
        
    Returns:
        None
    """
    client = get_openai_client(genai_provider)
    ...
    for annots_file_path, annots in load_annots(data_provider_obj, transformed_cuad_annots_path):
        ...
        try:
            for item in annots["qas"]:
                ...
                messages = [{"role": k, **v} for k,v in messages.items()]
            ...
            if err.status_code == 402:
                logger.error(f"Insufficient balance in the user account for making the request.")
                try:
                    log_user_balance()
                except Exception as ex:
                    logger.error(f"Failed to retrieve user's balance: {ex}")
            elif err.status_code == 429:
                logger.error(f"The DeepSeek API rate limit exceeded.")
            elif err.status.code == 401:
...
if __name__ == "__main__":
    ...
    review_annots(args.transformed_cuad_annots_path, 
                  args.reviewed_cuad_annots_path,
                  args.policies_path,
                  args.genai_provider)
